...
static void* RequestAddress(LibraryHandle handle, const char* functionName)
{
   ...
   if (p != NULL) 
      ...
      else sprintf(str, "%s : failed", functionName);
...
void InitializeImportedFunctions(LibraryHandle handle, TGetGPFunctionType getGPFunction) 
{
   ...
   RequestGPFunctionByName = getGPFunction; // This is only needed at the beginning so we can ask GP for the address of all the functions we want
