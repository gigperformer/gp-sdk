...
#include "GPMidiMessages.h"
   ...
   std::string GigPerformerFunctions::getPluginParameterName(const std::string & pluginHandle, int parameterIndex, bool useGlobalRackspace)
   {
        const int bufferLength = 1024;
        char returnBuffer[bufferLength] = {0};

        /* int actualLength = */ GP_GetPluginParameterName(fHandle, pluginHandle.c_str(), parameterIndex, returnBuffer, bufferLength, useGlobalRackspace);
        std::string result(returnBuffer);
        return result;
   }
   ...
   void  GigPerformerFunctions::sendMidiMessageToMidiOutDevice(const std::string & deviceName, const uint8_t* midiData, int length)
   {
   ...
   {
       GP_SendMidiMessageToMidiOutDevice(fHandle, deviceName.c_str(), (const uint8_t* )midiData.c_str(), (int)midiData.length());
   }
   ...
   void  GigPerformerFunctions::injectMidiMessageToMidiInputDevice(const std::string & deviceName, const uint8_t* midiData, int length)
   {
   ...
   {
       GP_InjectMidiMessageToMidiInputDevice(fHandle, deviceName.c_str(), (const uint8_t* )midiData.c_str(), (int)midiData.length());
   }



   void  GigPerformerFunctions::injectMidiMessageToMidiInputAlias(const std::string & rigManagerAlias, const uint8_t* midiData, int length)
   {
   ...
   {
       GP_InjectMidiMessageToMidiInputAlias(fHandle, rigManagerAlias.c_str(), (const uint8_t* )midiData.c_str(), (int)midiData.length());
   }
